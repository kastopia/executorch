...
class TestTurboQuant(unittest.TestCase):
    ...
    def test_eager_greedy_matches_baseline(self):
        """TurboQuant greedy decode matches baseline."""
        model_base = _make_model(turboquant=False)
        model_tq = _make_model(turboquant=True)

        def base_fn(tok, pos):
            with torch.no_grad():
                return model_base(tok, pos)

        def tq_fn(tok, pos):
            with torch.no_grad():
                return model_tq(tok, pos)

        base_tokens = _greedy_decode(base_fn, [1, 2, 3], 5)
        tq_tokens = _greedy_decode(tq_fn, [1, 2, 3], 5)
        self.assertEqual(base_tokens, tq_tokens)
